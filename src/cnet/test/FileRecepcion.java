...
import cnet.ClusterMemberInputStream;
import cnet.Log;
import cnet.ccopy.receive;
...
import java.util.TreeMap;
  ...
  */
   ...
   */
   public  FileRecepcion(receive receive, ClusterMemberID id_socket) throws IOException
   {
      super("FileRecepcion");
      this.setDaemon(true);
   ...
   */
   public FileRecepcion(receive receive,ClusterMemberInputStream id_socketIn )
  throws IOException
  ...
  public void run()
  {
   ...
   try
   {
      ...
      if(this.id_socketIn!=null)
        this.receive.removeFileRecepcion(this.id_socketIn);
   ...
   finally
   {
     ...
     //Log.log(receive.mn," FIN FileRecepcion FIABLE.","");
   }
  }
  ...
  private void receiveFile()  throws IOException
 {
     ...
     if (sFileName == null)
     {
      ...
      //Fichero temporal..
      this.file = File.createTempFile("cnet.ccopy"+System.currentTimeMillis(),".tmp");
     ...
     try
     {
      if(receive.getModo() == ClusterNet.MODE_DELAYED_RELIABLE
      || receive.getModo() == ClusterNet.MODE_RELIABLE)
      {
        // System.out.println("");
        // Log.log(receive.mn,"Iniciando la recepcion... de "+sFileName);

         lBytesLeidos = 0;
         byte[] bytes = new byte[1024*2];
         int contador = 0;

         while(receive.esActiva()/* && (lBytesLeidos < lFileSize) */&& (this.file!=null))
         {
              contador++;
            ...
            //{
                ...
                {
                  System.out.println("");
                  Log.log(receive.mn,"Recibido fin de flujo FTP");
                  break;
                }
                ...
                if((contador % 128) ==0)
                {
                  System.out.print("\r["+receive.mn+"] Bytes recibidos: "+lBytesLeidos);
                }
  ...
  finally
  {
     ...
     if(this.receive.getModo() == ClusterNet.MODE_DELAYED_RELIABLE
     || receive.getModo() == ClusterNet.MODE_RELIABLE)
     {
         this.receive.removeFileRecepcion(this.id_socketIn);
     }
     ...
     {
        this.receive.removeFileRecepcion(this.id_socket);
     }
     ...
     if(receive.esActiva() && this.file!=null)
          {
           ...
           if(MFTPfile.exists())
           {
             if(this.mensajeFileExists())
             {
              if (!MFTPfile.delete())
              {
                this.receive.error("No se ha podido eliminar el fichero: "+sFileName+newline+"1. Compruebe si tiene privilegios suficientes."+newline+"2.Compruebe que el fichero no está siendo usado por otra aplicación.");
                 //Eliminar temporal
                  this.file.delete();
                  this.file=null;
                return;
               ...
               {
                this.receive.error("No se ha podido renombrar a:"+sFileName+newline+"1. Compruebe si tiene privilegios suficientes."+newline+"2.Compruebe que el fichero no está siendo usado por otra aplicación.");
                //Eliminar temporal
                this.file.delete();
                  this.file=null;
               }
  ...
  */
  private void tirarBytes()throws IOException
  {
    ...
    try
    {
         ...
         Log.log(receive.mn,"Transferencia ya iniciada. Esperando siguiente para sincronizacion...");

         while(receive.esActiva() && (this.file!=null))
         {
 ...
 void receiveFile(long lFileSize, String sFileName)  throws IOException
 {
    ...
    Log.log(receive.mn,"Fichero: "+sFileName+" longitud: "+lFileSize);
...
*/
 ...
 private boolean receiveIDFTPMulticast() throws IOException
 {
   ...
   try
   {
    ...
    System.out.println("");
    System.out.println("");
    Log.log(receive.mn,"Iniciando la recepcion runGUI...");
    //System.out.println("--------------------------------------");
 ...
 private String receiveFileName() throws IOException
 {
   ...
   try
   {
        ...
        Log.log(receive.mn,"Recibiendo fichero: "+sFileName+" del emisor: "+this.id_socketIn.getID_Socket());
   }
 ...
 private long receiveFileSize() throws IOException
 {
   ...
   try
   {
        ...
        Log.log(receive.mn,"Tamaño: "+lFileSize);
   }
...
//==========================================================================
 ...
 public static long parseFileSize(Buffer buf) throws IOException
 {
   ...
   //==========================================================================
 /**
  * Comprueba Identificador de runGUI ClusterNet v1.0
 ...
 public static boolean parseIDFTPMulticast(Buffer buf) throws IOException
 {
   ...
   {
      //Leer Magic
      if(buf.getInt(0) != receive.MAGIC)
       return bOK;

      if( buf.getInt(4) != receive.VERSION)
        return bOK;
 ...
 public void stopTransferencia()
 {
   ...
   this.file = null;
    Log.log(receive.mn,"Recepción cancelada por el usuario.");
 }
 }
