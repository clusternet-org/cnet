...
import cnet.old.ID_SocketInputStream;
...
import java.util.TreeMap;
  ...
  */
   ...
   */
   public  FileRecepcion(ProtocolcFTP protocolcFTP, ID_Socket id_socket) throws IOException
   {
      ...
      this.protocolcFTP = protocolcFTP;
  ...
  public void run()
  {
   ...
   try
   {
      ...
      if(this.id_socketIn!=null)
        this.protocolcFTP.removeFileRecepcion(this.id_socketIn);
  ...
  private void receiveFile()  throws IOException
 {
     ...
     if (sFileName == null)
     {
      ...
      //Fichero temporal..
      this.file = File.createTempFile("cnet.ftp"+System.currentTimeMillis(),".tmp");
     ...
     try
     {
      if(protocolcFTP.getModo() == ClusterNet.MODE_DELAYED_RELIABLE
      || protocolcFTP.getModo() == ClusterNet.MODE_RELIABLE)
      {
         cFtp.getFTP().insertRecepcionString("Initializing reception of "+sFileName,null);

         lBytesLeidos = 0;
         byte[] bytes = new byte[1024*2];

         while(protocolcFTP.esActiva()/* && (lBytesLeidos < lFileSize) */&& (this.file!=null))
         {
  ...
  finally
  {
     ...
     //Eliminar este objeto del treemap en la clase ProtocolcFTP
     if(this.protocolcFTP.getModo() == ClusterNet.MODE_DELAYED_RELIABLE
     || protocolcFTP.getModo() == ClusterNet.MODE_RELIABLE)
     {
         this.protocolcFTP.removeFileRecepcion(this.id_socketIn);
     }
     ...
     if(protocolcFTP.esActiva() && this.file!=null)
          {
           ...
           if(MFTPfile.exists())
           {
             if(this.mensajeFileExists())
             {
              if (!MFTPfile.delete())
              {
                this.protocolcFTP.getFTP().error("The file can't be deleted: "+sFileName+newline+"1. Check you hace privilegies."+newline+"2.Check the file is not used by another application.");
                 //Eliminar temporal
                  this.file.delete();
                  this.file=null;
                return;
               ...
               {
                this.protocolcFTP.getFTP().error("The file can't be renamed:"+sFileName+newline+"1. Check you hace privilegies."+newline+"2.Check the file is not used by another application.");
                //Eliminar temporal
                this.file.delete();
                  this.file=null;
               }
  ...
  */
  private void tirarBytes()throws IOException
  {
    ...
    try
    {
         ...
         while(protocolcFTP.esActiva() && (this.file!=null))
         {
...
//==========================================================================
 ...
 public static boolean parseIDFTPMulticast(Buffer buf) throws IOException
 {
   ...
   {
      //Leer Magic
      if(buf.getInt(0) != ProtocolcFTP.MAGIC)
       return bOK;

      if( buf.getInt(4) != ProtocolcFTP.VERSION)
        return bOK;
 ...
 public void stopTransferencia()
 {
   ...
   this.file = null;
  cFtp.getFTP().insertRecepcionString("Reception canceled by the user.","icono_informacion");
 }
 }
