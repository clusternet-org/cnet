...
import java.util.TreeMap;
  ...
  */
   ...
   * Constructor.
   */
   public  FileEmision(ProtocolcFTP protocolcFTP,File file,Icon icon) throws IOException
   {
      this.file = file;
      ...
      if(this.protocolcFTP.getModo() == ClusterNet.MODE_DELAYED_RELIABLE
        || this.protocolcFTP.getModo() == ClusterNet.MODE_RELIABLE )
      {
        //Flujo de salida Multicast ....
        out = this.protocolcFTP.getSocket().getClusterOutputStream();
        if(out == null)
        {
   ...
   */
   void sendFile() throws IOException
   {
     ...
     try
     {
          ...
          //Buffer
          byte[] aBytes =  new byte[ProtocolcFTP.TAMAÑO_ARRAY_BYTES];
          ...
          for(lBytesTransmitidos = 0; lBytesTransmitidos<lFile && this.protocolcFTP.esActiva();)
          {
              //Leer bytes...
              ...
              if(this.bStop == true || !this.protocolcFTP.esActiva())
              {
                 file = null;
 ...
 private void sendBytes(byte[] aBytes,int iBytes) throws IOException
 {
    ...
    else
    {
      ...
      this.protocolcFTP.getDatagramSocket().send(buf);
    }
 }
 ...
 private void sendCabeceraFTP(long lSize,String sFileName) throws IOException
 {
     ...
     //ID_FTP
      buf.addInt(ProtocolcFTP.MAGIC,0);
      buf.addByte((byte)ProtocolcFTP.VERSION,4);
