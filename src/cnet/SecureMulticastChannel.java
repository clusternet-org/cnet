...
//----------------------------------------------------------------------------
//
//	File: SecureMulticastChannel.java  1.0 13/09/99
//
//
//	Description: Clase SecureMulticastChannel.
//
// 	Authors: 
 ...
 * datos que transmita a través de ClusterNet.</STRONG>.
 *
 * @version  1.0
...
class SecureMulticastChannel extends MulticastChannel
{
  /** Dirección Multicast */
  ...
  /** El objeto ChannelCallback que implementa el método callback si el flag
  * callback está activado */
  private ChannelCallback callbackObject = null;
   ...
   * @exception ClusterNetExcepcion Error creando el socket o uniéndose (join) al
   *  grupo multicast o estableciendo la interfaz de salida de los datos
   *  multicast.
   */
  SecureMulticastChannel(Address grupoMcast,  Address ifMcast
   ,javax.crypto.Cipher cipher, javax.crypto.Cipher unCipher) throws ClusterNetExcepcion
  {
    final String  mn = "SecureMulticastChannel.CanalMulticastSeguro(Address,Address,javax.crypto.Cipher)";
    ...
    if(cipher == null)
      throw new ClusterNetExcepcion("Objeto Cipher Nulo");

    if(unCipher == null)
      throw new ClusterNetExcepcion("Objeto unCipher Nulo");
    ...
    try
    {
      ...
      //Log.debug(Log.CANAL_MULTICAST,"SecureMulticastChannel.CanalMulticastSeguro","Socket Multicast "+grupoMcast+"--> OK");


      if (this.socketObj == null)
         throw new ClusterNetExcepcion("Error en la creación del socket multicast. Socket nulo.");

      //Buffer Emision a tamaño máximo... (64Kb)
      this.socketObj.setSendBufferSize    (ClusterNet.SIZE_BUFFER_SOCKET_EMISION);
      this.socketObj.setReceiveBufferSize (ClusterNet.SIZE_BUFFER_SOCKET_RECEPCION);
          ...
          (!ifMcast.isMulticastAddress()))
      {
          ...
          //Log.debug(Log.CANAL_MULTICAST,"SecureMulticastChannel.CanalMulticastSeguro","Socket Multicast Interface "+ifMcast+"--> OK");
      }
      ...
      if (grupoMcast.isMulticastAddress())
      {
        ...
        //Log.debug(Log.CANAL_MULTICAST,"SecureMulticastChannel.CanalMulticastSeguro","Socket Multicast join --> OK");
      }
    ...
    catch(ClusterNetInvalidParameterException ioe)
    {
      throw new ClusterNetExcepcion(mn,ioe.getMessage());
    }
  ...
  public synchronized void close()
  {
   ...
   //Log.debug(Log.CANAL_MULTICAST,"SecureMulticastChannel.close","Socket Multicast close --> OK");
  }
...
long nBytesEnviados = 0;
  public synchronized void send(Buffer buf,  byte TTL)
   throws ClusterNetExcepcion, ClusterNetInvalidParameterException, IOException
  {
    final String    mn = "SecureMulticastChannel.send";
    ...
    if (this.socketObj == null)
    {
      throw new ClusterNetExcepcion(mn, "El socket no está abierto.");
    }
    ...
    //
    if (buf == null)
      throw new ClusterNetInvalidParameterException(mn, "Argumentos requeridos erróneos. Buffer nulo.");


    if ((buf.getBuffer() == null) || (buf.getLength() <= 0))
    {
        throw new ClusterNetInvalidParameterException(mn, "Buffer erróneo.");
    }
    ...
    try
    {
     if (this.sendPacket == null)
     {
      ...
      if (this.sendPacket == null)
      {
        throw new ClusterNetExcepcion(mn, "No se pudo crear el datagrama de emisión.");
      }
     }
     else
     {
        ////Log.debug(Log.CANAL_MULTICAST,"SecureMulticastChannel.send","Datagrama: longitud:"+buf.getLength()+" destino: "+this.addressMcast);
        byte[] aCifrado = this.cipher.doFinal(buf.getBuffer(),0,buf.getLength());
  ...
  public void receive(Buffer buf, Address src)
   throws ClusterNetExcepcion, ClusterNetInvalidParameterException, IOException
  {
    final String    mn = "SecureMulticastChannel.receive(Buffer,Address)";
    Buffer bufCodificado = null;

    //Log.debug(Log.CANAL_MULTICAST,"SecureMulticastChannel.receive","Socket Multicast receive.");
    ...
    if (this.callback)
    {
      throw new ClusterNetExcepcion(mn,"No se puede utilizar el método receive. Existe un método callback registrado.");
    }
    ...
    {
      throw new ClusterNetInvalidParameterException(mn, "Parámetros inválidos.");
    }
    if ((buf.getBuffer() == null) || (buf.getMaxLength() <= 0))
    {
      throw new ClusterNetExcepcion(mn, "Buffer nulo o de tamaño erróneo.");
    }
    ...
    if (this.recvPacket == null)
    {
      ...
      if (this.recvPacket == null)
      {
        throw new ClusterNetExcepcion(mn, "No se pudo obtener el datagrama de recepción.");
      }
    }
    else
    {
  ...
  public int getMaxPacketSize()
  {
    ...
    //
    return (ClusterNet.TPDU_MAX_SIZE );
  }
  ...
  {
    final String  mn = "SecureMulticastChannel.setTTL(byte)";
   ...
   */
  public synchronized void setCallback(ChannelCallback obj, int arg)
   throws ClusterNetExcepcion,ClusterNetInvalidParameterException
  {
    final String  mn = "SecureMulticastChannel.setCallback(ChannelCallback,int)";
    ...
    if (this.socketObj == null)
    {
      throw new ClusterNetExcepcion(mn, "Socket no abierto.");
    }

    if (obj == null)
    {
      throw new ClusterNetInvalidParameterException(mn, "Objeto callback nulo.");
    }
    ...
    //Log.debug(Log.CANAL_MULTICAST,"SecureMulticastChannel","Callback registrado: obj:"+obj+" arg:"+arg);
  ...
  {
    final String  mn = "SecureMulticastChannel.disableCallback()";
...
/**
 * Thread de recepción del Channel Multicast. Thread demonio.
...
class MulticastThreadSeguro extends Thread
{
  ...
  private SecureMulticastChannel  canalObj = null;

  /** El socket multicast sobre el que se recibirán los datos. */
  private MulticastSocket  socketObj = null;

  /** El objeto ChannelCallback */
  ChannelCallback  callbackObject = null;
   ...
   */
  public MulticastThreadSeguro(SecureMulticastChannel canal, MulticastSocket sock)
  {
  ...
  public void run()
  {
    ...
    //try{
      abytesCodificado = new byte[ClusterNet.TPDU_MAX_SIZE];
    /*}
   ...
   //     Log.log ("NUMERO BYTES RECIBIDOS ACUMULADOS : " + nBytesRecibidos,"");
      ...
      if (bufferReturn.getLength() > 0)
      {
        ...
        if (this.callbackObject != null)
        {
          ...
          catch (ClusterNetExcepcion e){Log.log (mn,e.toString());}
          */
         }
