...
public class GestionAsentNeg implements TimerHandler
{
 // ATRIBUTOS
  ...
  private TimerHandler timerHandlerFinTAsentNeg = new TimerHandler ()
       {
         /**
          * Función utilizada por el  {@link ClusterTimer} para indicar el evento
          * registrado.
  ...
  * @param socketPTMFImpParam socketClusterNetImp al que está vinculada.
  */
  public GestionAsentNeg (SocketClusterNetImp socketPTMFImpParam)
  {
    ...
    this.random = new Random (ClusterTimer.tiempoActualEnMseg ());
       ...
       if (this.listaID_TPDUNoRecibidos.size()==0) // No estamos esperando.
        {
         ...
         this.lTRandomAsentNeg = (this.random.nextInt (ClusterNet.MAX_TRANDOM_ASENT_NEG-ClusterNet.T_BASE)+ClusterNet.T_BASE)
                           % ClusterNet.MAX_TRANDOM_ASENT_NEG;
         this.socketClusterNetImp.getTemporizador().registrarFuncion (this.lTRandomAsentNeg,this,
  ...
  public void actualizar (ListaOrdID_TPDU listaAsentNeg,boolean bFinRTTParam)
  {
    ...
    if (bFinRTTParam
        ||
        (this.socketClusterNetImp.getTemporizador().getPorcentajeRTTActual() > 50) )// MAYOR DEL 50 %
      {
        // Actualizar la lista
   ...
   if (lista.size()>0)
    {
     ...
     this.socketClusterNetImp.getTemporizador().registrarAvisoRTT (this.timerHandlerFinTAsentNeg,
     ...
     if (this.listaID_TPDUNoRecibidos.size()==0)
      {
       ...
       this.socketClusterNetImp.getTemporizador().cancelarFuncion (this,this.iEVENTO_FIN_T_RANDOM_ASENT_NEG);
      }
       ...
       if (reg.listaID_TPDU.size()==0) // Se ha quedado vacio.
         {
          ...
          this.socketClusterNetImp.getTemporizador().cancelarRTT (timerHandlerFinTAsentNeg,reg.getIdentificador());
     ...
     if (lista.size()>0)
       {
            ...
            while (iteradorID_Socket.hasNext())
             {
              ...
              this.socketClusterNetImp.getCanalUnicast().send (tpduHNACK.construirTPDUHNACK(),
                                                         dirUnicastDestino);
        ...
        /* Se manda por HNACK Si:
                ...
                B - contadorRtx >= 1
        */
        if (this.socketClusterNetImp.getCGLThread().numeroVecinos()==0
            ||
            registroAsentNeg.iContadorRtx >=1)
            {
             /* Puede dar problemas de espacio. */
             TPDUHNACK tpduHNACK = TPDUHNACK.crearTPDUHNACK (this.socketClusterNetImp,lista);
             ...
             */
             this.socketClusterNetImp.getCanalMcast().send (tpduHNACK.construirTPDUHNACK());
        ...
        */                    // Este y el de abajo son TPDUNACK no HNACK
        TPDUNACK tpduNACK = TPDUNACK.crearTPDUNACK (this.socketClusterNetImp,lista);
        ...
        */
        this.socketClusterNetImp.getCanalMcast().send (tpduNACK.construirTPDUNACK(),
                     (byte)this.socketClusterNetImp.getCGLThread().getTTLSocketsGL());
