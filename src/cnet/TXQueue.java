 ...
 * datos y aptos para procesar y encolar en la Window de Emisión.<br>
      ...
      while(offset < longArray)
      {
        ...
        if(this.socket.getModo() == ClusterNet.MODE_RELIABLE || this.socket.getModo() == ClusterNet.MODE_SEMIRELIABLE)
        {
          ...
          if( longArray - offset >  ClusterNet.MTU)
            longNuevoBuffer = ClusterNet.MTU; //Buffer máximo
          else
          {
        ...
        else
        {
          ...
          {
            longNuevoBuffer = ClusterNet.TPDU_MAX_SIZE_PAYLOAD; //Buffer máximo
