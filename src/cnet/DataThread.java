...
//----------------------------------------------------------------------------
//
//	Fichero: DataThread.java  1.0 13/10/99
//
...
//                   <ul>
//                       <li>{@link ClusterNet#MODE_RELIABLE}</li>
//                       <li>{@link ClusterNet#MODE_DELAYED_RELIABLE}</li>
//                       <li>{@link ClusterNet#MODE_NO_RELIABLE}</li>
//                       <li>{@link ClusterNet#MODE_NO_RELIABLE_ORDERED}</li>
//                    </ul>
//                   Basada en la especificación del protocolo ClusterNet.
//
// 	Authors: 
...
class DataThread  extends Thread implements TimerHandler,ClusterMemberListener,ClusterGroupListener
{
  ...
  /** Window de emisión con los TPDUDatosNormal que tienen que ser enviados. <br>
      Sólo utilizado si este socket es fuente de datos.*/
  TxWindow  txWindow = null;
  ...
  /** Número de secuencia del siguiente TPDU datos a añadir a la ventana de emisión. <br>
      Sólo utilizado si este socket es fuente de datos.*/
  private SecuenceNumber NSecuencia = null;
  ...
  /** Número de TPDU que se pueden enviar entre dos TPDU's con el bit ACK activado.
      ...
      Sólo utilizado si este socket es fuente de datos. */
  private int iACKCadaXTPDUS = ClusterNet.ACKS_CADA_X_TPDUS;
  ...
  /** Ratio que no tiene que ser superado para el envío de nuevos datos de
      usuario. <br>
      Sólo utilizado si este socket es fuente de datos.*/
  private long lRatioUsuario_Bytes_x_seg = ClusterNet.RATIO_USUARIO_BYTES_X_SEG;
  ...
  /** Tiempo máximo de inactividad de este socket.<br>
      Sólo utilizado si este socket es fuente de datos.*/
  private long lT_TIME_OUT_INACTIVIDAD = (ClusterNet.TIEMPO_MAX_INACTIVIDAD_EMISOR * 2) / 3  ;
  ...
  private int iMODO_FIABILIDAD = ClusterNet.MODE_RELIABLE;
  ...
  // y que son disparados por el ClusterTimer.
  //-----------------------------------------------------------------------
   ...
   * @param socketPTMFImpparam referencia al socketClusterNetImp que crea este thread.
   ...
   * @see ClusterNet
   */
  public DataThread (SocketClusterNetImp socketPTMFImpparam,int iModo_fiabilidad)
  {
    super("DataThread");
    //PRIORIDAD..
    ...
    final String mn = "DataThread.DatosThread (ClusterNet)";

    this.iMODO_FIABILIDAD = iModo_fiabilidad;
    this.socketClusterNetImp = socketPTMFImpparam;
    this.random = new Random (System.currentTimeMillis ());
    this.id_SocketLocal = this.socketClusterNetImp.getAddressLocal().toID_Socket();

    if ((iModo_fiabilidad == ClusterNet.MODE_RELIABLE) || (iModo_fiabilidad == ClusterNet.MODE_DELAYED_RELIABLE))
      {
       ...
       this.gestionAsentNeg = new GestionAsentNeg (this.socketClusterNetImp);
       ...
       try
        ...
        this.tablaAsentimientos = new TablaAsentimientos (this);
           ...
           {
            // ALEX:
            this.socketClusterNetImp.sendPTMFEventError ("No se ha podido crear las estructuras de datos necesarias.");
            Log.exit (-1);
           }
    ...
    if (this.iMODO_FIABILIDAD == ClusterNet.MODE_NO_RELIABLE_ORDERED)
  ...
  //==========================================================================
  /**
   * Devuelve true si se ha llamado al close de ClusterNet.
   * @see ClusterNet
   */
  ...
  public void run()
  {
    final String    mn = "DataThread.run()";
    boolean         bForever;
    Buffer          buf;
    ...
    while (bForever)
    {
      ...
      {
          Log.debug(Log.DATOS_THREAD,mn,"Fin del hilo DataThread");
          return;
      }
      ...
      //-------------------------------------------------------------------
      if (!this.socketClusterNetImp.isGrupoMcastActivo())
      {
        this.socketClusterNetImp.getTemporizador().sleep(1);
      ...
      if (this.bComprobarInactividadEmisores)
       {
        ...
        this.socketClusterNetImp.getTemporizador().registrarFuncion (ClusterNet.TIEMPO_COMPROBACION_INACTIVIDAD_EMISORES,
                                       this,
          ...
          while (iteradorID_Socket.hasNext())
           {
            ...
            if (this.tablaID_SocketsEmisores.entregaDatosUsuario (id_socketNext,
                                         this.socketClusterNetImp.getColaRecepcion()))
                {
      ...
      //    ClusterNet.MAX_TPDU_PROCESAR_CONSECUTIVAMENTE TPDU datos consecutivamente,
      //    es decir, antes de enviar ningún TPDU.
      //-------------------------------------------------------------------
      iContadorIteraciones = ClusterNet.MAX_TPDU_PROCESAR_CONSECUTIVAMENTE;
      while ( (!this.socketClusterNetImp.vectorRegistroDatos.isEmpty())
              &&
              (iContadorIteraciones >= 1)  )
       { // Analizar los TPDUDatos recibidos
        tpduDatos = (TPDUDatos)this.socketClusterNetImp.vectorRegistroDatos.remove (0);
     ...
     if (this.txWindow != null)
     { // Este socket es fuente de datos.
         if (this.txWindow.estaLlena()
             &&
             this.socketClusterNetImp.vectorRegistroDatos.isEmpty())
         {
           iInactividadEmisor++;
        ...
        {// Este socket no es fuente de datos
         if (this.socketClusterNetImp.vectorRegistroDatos.isEmpty()
              &&
             ...
             this.vectorMACK.isEmpty())
          {
              ...
              //   {
                           this.socketClusterNetImp.getTemporizador().sleep(7);
  ...
  private void eliminarID_SocketFuente (ClusterMemberID id_socketFuente,String msg)
  {
   final String mn = "DataThread.eliminarID_SocketFuente (id_socketFuente,msg)";
         ...
         if (this.tablaID_SocketsEmisores.removeID_Socket (id_socketFuente))
          {
           ...
           if (msg!=null)
             this.socketClusterNetImp.sendPTMFEventConexion("[" + id_socketFuente + "]" + msg);
          }
          else return;
        }
    ...
    this.socketClusterNetImp.getColaRecepcion().remove (id_socketFuente);
  }
 ...
 private  void procesarTPDUDatos (TPDUDatos tpduDatos)
  ...
  {
   final String mn = "DataThread.procesarTPDUDatosFiable (tpduDatos)";
  ...
  {
   final String mn = "DataThread.procesarTPDUDatosNoFiable (tpduDatos)";
  ...
  private void procesarTPDUDatosNormalFiable (TPDUDatosNormal tpduDatosNormal)
  {
   ...
   //=======================================================================
   // 2. Obtener Window de recepción parar guardar el TPDU

   // Guardar el TPDU Datos en la ventana de recepción del socket que lo envió.
   RxWindow vtaRecp = this.tablaID_SocketsEmisores.getVentanaRecepcion (id_SocketSrc);
   ...
   {    // No esta registrado como emisor.
           ...
           (tpduDatosNormal.getNumeroRafaga()==1) )
        {
            try{
              ...
              this.socketClusterNetImp.sendPTMFEventConexion("Comienzo emisión del id_socket: " + id_SocketSrc);

              // Puede ser que se haya enviado un asentimiento negativo para
              // el TPDU primero (SecuenceNumber.LIMITE_INFERIOR).
              ID_TPDU id_tpduAux = new ID_TPDU (id_SocketSrc,SecuenceNumber.LIMITEINFERIOR);
   ...
   */
   ...
   {
     final   String mn = "DataThread.procesarTPDUDatosRtx ";
     boolean bRtxPorPadreJerarquico = false;
     boolean bRtxPorMismoGrupoLocal = false;
     boolean bNoLoTenia = false;

     SecuenceNumber nSec = tpduDatosRtx.getID_TPDUFuente().getNumeroSecuencia();
     ...
     bRtxPorMismoGrupoLocal = tpduDatosRtx.getIDGL().equals (
                                      this.socketClusterNetImp.getCGLThread().getIDGL());

     if (!bRtxPorMismoGrupoLocal)
     // Ver si viene de un padre jerárquico
         {

          TreeMap treeMapIDGLPadres = this.socketClusterNetImp.getCGLThread().getCGPadres
    ...
    RxWindow vtaRec = this.tablaID_SocketsEmisores.getVentanaRecepcion
        ...
        if (this.iMODO_FIABILIDAD == ClusterNet.MODE_DELAYED_RELIABLE)
          {
           ...
           // han asentido.
           // Si el modo es ClusterNet.PTMF_FIABLE_RETARDADO entonces, si figuramos en
        ...
        if (tpduDatosRtx.getIR() && tpduDatosRtx.getNumeroRafagaFuente()==1)
         {
          ...
          this.socketClusterNetImp.sendPTMFEventConexion("Comienzo de emsion del id_socket: " +
                                tpduDatosRtx.getID_TPDUFuente().getID_Socket());
         }
        else
         {
          Log.log (mn,"Comienzo  de emisión RETRASADA de la fuente: " + tpduDatosRtx.getID_TPDUFuente().getID_Socket());
          this.socketClusterNetImp.sendPTMFEventConexion("Comienzo de emsion retrasada del id_socket" +
                                tpduDatosRtx.getID_TPDUFuente().getID_Socket());
         }
    ...
    if (bRtxPorMismoGrupoLocal)
       {
        SecuenceNumber nSecInicRafaga = this.tablaCGLocales.getNumSecInicialRafaga
                                 ( tpduDatosRtx.getID_TPDUFuente().getID_Socket(),
       ...
       if (tpduDatosRtx.getACK ())
         {
          ...
          if (listaIDGLNoAsentidos!=null)
           if (listaIDGLNoAsentidos.containsKey (this.socketClusterNetImp.getCGLThread().getIDGL()))
  ...
  if (!this.tablaAsentimientos.contieneID_TPDU (tpduACK.getID_TPDUFuente()))
     {
      ...
      if (tpduACK.getID_TPDUFuente().getID_Socket().equals (this.id_SocketLocal))
       { // Buscar en la ventana de emisión
         ...
         if (!this.txWindow.nSecEntreInicialyFinal(tpduACK.getID_TPDUFuente().
                                        getNumeroSecuencia().tolong()))
           return;
      }
      else // Buscar en la ventana de recepción
          {
            RxWindow vtaRecp = this.tablaID_SocketsEmisores.
  ...
  {
  final String mn = "DataThread.procesarTPDUNACK";
  TPDUDatosNormal tpduDatosNormal;
   ...
   if (!(tpduNACK.getIDGL().equals (this.socketClusterNetImp.getCGLThread().getIDGL())))
         return;
   ...
   while (iterador.hasNext())
    {
     ...
     RxWindow vtaRecep = this.tablaID_SocketsEmisores.getVentanaRecepcion(id_SocketSrc);
  ...
  {
   final String mn = "DataThread.procesarTPDUHNACK";
   TPDUDatosNormal tpduDatosNormal;
   ...
   while (iterador.hasNext())
    {
     ...
     if (tpduHNACK.recibidoPorUnicast)// Recibido por unicast
      {
       // Por unicast sólo me pueden pedir los que yo emito (txWindow)
       if (id_SocketSrc.equals (this.id_SocketLocal))
     ...
     bCondicion = (treeMapIDGLHijos==null?this.socketClusterNetImp.getIDGL().equals(idglEmisor):
                                    ...
                                    ||
                                    this.socketClusterNetImp.getIDGL().equals(idglEmisor));
  ...
  {
  final String mn = "DataThread.procesarTPDUMACK";
  ...
  if (tpduMACK.getIDGL().equals (this.socketClusterNetImp.getCGLThread().getIDGL()))
   ...
   */
   ...
   {
    final String mn = "DataThread.procesarTPDUHSACK";
      ...
      if (tpduHSACK.getID_TPDUFuente().getID_Socket().equals (this.id_SocketLocal))
       {
         ...
         // Buscar en la ventana de emisión
         if (!this.txWindow.nSecEntreInicialyFinal (tpduHSACK.getID_TPDUFuente().
                                getNumeroSecuencia().tolong()))
           return;
       }
       else // Buscar en la ventana de recepción
   ...
   */
   ...
   {
    final String mn = "DataThread.procesarTPDUHACK";
      ...
      if (tpduHACK.getID_TPDUFuente().getID_Socket().equals (this.id_SocketLocal))
       {
         if (this.txWindow==null)
           return;
         // Buscar en la ventana de emisión
         if (!this.txWindow.nSecEntreInicialyFinal (tpduHACK.getID_TPDUFuente().
                                getNumeroSecuencia().tolong()))
           return;
       }
      else // Buscar en la ventana de recepción
   ...
   if (bResult) // Se ha enviado algún TPDU
    {
     ...
     this.lT_Min_Entre_Envios = ClusterNet.T_MIN_ENTRE_ENVIOS;
     this.socketClusterNetImp.getTemporizador().registrarFuncion (ClusterNet.T_MIN_ENTRE_ENVIOS,  /* mseg */
                                    this,                     /* TimerHandler */
  ...
  {
   final String mn = "DataThread.enviarTPDUFiable()";
  ...
  {
   final String mn = "DataThread.enviarTPDUDatosNormalFiable";
   ...
   TPDUDatosNormal tpduDatosNormal = this.txWindow.siguienteTPDUDatosNormal ();
   ...
   if (tpduDatosNormal!=null) // 1º if
       {
        ...
        if (tpduDatosNormal.getIR())
          ...
          this.socketClusterNetImp.getCanalMcast().send (bufAEnviar);

          //ALEX:  depuración, comentar
          if(tpduDatosNormal.getACK())
          {
            Log.debug(Log.TPDU_DATOS,"DataThread.enviarTPDUDatosNormalFiable","--> TPDU DatosNormal (con ACK):"+tpduDatosNormal.getID_TPDU());
            //Log.debug(Log.DATOS_THREAD,"",""+tpduDatosNormal.getBufferDatos());
          }
          else
          {
            Log.debug(Log.TPDU_DATOS,"DataThread.enviarTPDUDatosNormalFiable","--> TPDU DatosNormal:"+tpduDatosNormal.getID_TPDU());
            //Log.debug(Log.DATOS_THREAD,"",""+tpduDatosNormal.getBufferDatos());
          }
  ...
  private void lanzarTimeOutInactividad ()
  {
   ...
   if (!bTime_out_inactividad)
    this.socketClusterNetImp.getTemporizador().cancelarFuncion(this,this.iEVENTO_TIME_OUT_INACTIVIDAD);

   bTime_out_inactividad = false;

   // Lanzar temporizador para bTime_out_inactividad
   this.socketClusterNetImp.getTemporizador().registrarFuncion(this.lT_TIME_OUT_INACTIVIDAD,
                                     this,
  ...
  {
    final String mn = "DataThread.pasarBufferUsuarioAVentanaEmisionFiable";
    TXQueue tXQueue = this.socketClusterNetImp.getColaEmision();
    Buffer buffer = null;
    ...
    try
    {
     if (this.txWindow==null)
     {
          ...
          if((tXQueue == null) || (tXQueue.getTamaño()<=0))
              return;
          ...
          if (ClusterNet.ACKS_CADA_X_TPDUS >= this.txWindow.getTamañoMaximoVentana())
               this.iACKCadaXTPDUS = this.txWindow.getTamañoMaximoVentana() - 1;
          else this.iACKCadaXTPDUS = ClusterNet.ACKS_CADA_X_TPDUS;
     ...
     if (this.txWindow==null || this.txWindow.estaLlena())
      {
     ...
     if((tXQueue != null) && (!tXQueue.vacia()))
      {
       // Obtener un buffer de la cola de emisión.
       regBuf = tXQueue.remove();
       if(regBuf == null)
          Log.debug(Log.DATOS_THREAD,"DataThread --> RegistroBuffer NULO¡¡¡¡ ","");

       if(regBuf.getBuffer()== null)
          Log.debug(Log.DATOS_THREAD,"DataThread -->  Buffer NULO¡¡¡¡ ","");
     ...
     */
     if ( buffer == null
          ...
          ||
          this.iTPDUsRafaga == (ClusterNet.TPDUS_POR_RAFAGA-1) )
      ...
      if (this.iTPDUsRafaga >= ClusterNet.TPDUS_POR_RAFAGA)
       {
        this.bSetIR = true;
        this.iNRafaga ++;
      ...
      /* Se tiene que crear un tpdu, aunque sea null, para indicar el fin de
       ...
       if (buffer!=null)
         {
          if (this.bFIN_EMISION && tXQueue.getTamaño()<=0)
                 {
        ...
        TPDUDatosNormal tpdu = TPDUDatosNormal.crearTPDUDatosNormal
            (/*SocketClusterNetImp*/this.socketClusterNetImp,
             ...
             /*SecuenceNumber nSec*/this.NSecuencia,
             /*Buffer datos*/buffer);

       // -Añadir el TPDU a la ventana de Emisión
       this.txWindow.addTPDUDatosNormal(this.NSecuencia.tolong(),tpdu);
       ...
       if (this.bSetACK)
         {
          ...
          // -Cancelar ClusterTimer de Time Out último tpdu sin ack
          this.socketClusterNetImp.getTemporizador().cancelarFuncion(this,this.iEVENTO_TIME_OUT_ULTIMO_TPDU_SIN_ACK);

          // -Lanzar temporizador para Time Out último tpdu sin ack
          this.socketClusterNetImp.getTemporizador().registrarFuncion(ClusterNet.T_MAX_TPDU_SIN_ACK,
                                        this,
   ...
   * @throws ClusterNetInvalidParameterException Lanzada si hay un error en la creación
   * del número de secuencia inicial al crear la ventana.
   */
  private boolean crearVentanaEmision() throws ClusterNetInvalidParameterException
   {
    final String mn = "DataThread.crearVentanaEmision()";
     ...
     if ((this.NSecuencia==null)||(this.NSecuencia.tolong() <= 0))
       {
        this.N_SECUENCIA_INICIAL = this.socketClusterNetImp.getTemporizador().tiempoActualEnMseg();
        this.N_SECUENCIA_INICIAL = (this.N_SECUENCIA_INICIAL << 33) >>> 33;
        this.NSecuencia = new SecuenceNumber(this.N_SECUENCIA_INICIAL);
       }
     ...
     // Crear la ventana de emisión.
     this.txWindow = new TxWindow(ClusterNet.TAMAÑO_VENTANA_EMISION,N_SECUENCIA_INICIAL);

     if (this.iMODO_FIABILIDAD == ClusterNet.MODE_DELAYED_RELIABLE)
       {
        ID_TPDU id_tpdu = new ID_TPDU (this.id_SocketLocal,new SecuenceNumber (this.N_SECUENCIA_INICIAL));
        this.treeMapNuevosEmisoresASincronizar.put (this.id_SocketLocal,id_tpdu);
       }

    this.lNBytesUsuarioEnviados = 0;
    this.socketClusterNetImp.getTemporizador().registrarFuncionPeriodica (
                                         this, // TimerHandler
  ...
  {
    final String mn = "DataThread.pasarBufferUsuarioAVentanaEmisionNoFiable";
    TXQueue tXQueue = this.socketClusterNetImp.getColaEmision();
    Buffer buffer = null;
    RegistroBuffer regBuf = null;
    ...
    try
    {
     ...
     {
      if (regBuf != null)
         this.socketClusterNetImp.getColaEmision().addFirst (regBuf);
     }
    catch(ClusterNetExcepcion e)
     {
      if (regBuf != null)
  ...
  {
   final String mn = "DataThread.enviarTPDUDatosNormalNoFiable";
   ...
   if (tpduDatosNormal!=null)
       {
        try {
          ...
          // emisión.
          this.txWindow.removeTPDUDatosNormal (tpduDatosNormal.getNumeroSecuencia().tolong());
        }catch (ClusterNetInvalidParameterException pie)
             {
  ...
  {
   final String mn = "DataThread.RtxTPDUFinRTT";
   ...
   SecuenceNumber nSecMayor2MitadRTT = null;
...
/*      // Coger de la lista this.treeMapIDs_TPDU2MitadRTT el mayor de este host
      nSecMayor2MitadRTT = (SecuenceNumber)this.treeMapIDs_TPDU2MitadRTT.get
...
*/
   ...
   * número de secuencia inferior posible ({@link SecuenceNumber#LIMITEINFERIOR}),
  ...
  private TPDUDatosNormal buscarTPDUEnVentana (ID_TPDU id_tpdu)
  {
   ...
   if (id_socket.equals (this.id_SocketLocal))
      window = this.txWindow;
   else
      window = this.tablaID_SocketsEmisores.getVentanaRecepcion (id_socket);

   if (window == null)
        return null;

   if (SecuenceNumber.LIMITEINFERIOR.equals(nSec))
    {
     TPDUDatosNormal tpduDatosNormal = window.getTPDUDatosNormal
                                                  (window.getNumSecInicial());
     if (tpduDatosNormal!=null)
    ...
    return window.getTPDUDatosNormal (nSec.tolong());
  }
  ...
  {
   final String mn = "DataThread.enviarTPDUDatosSolicitadosRtx";
  ...
  {
   final String mn = "DataThread.enviarAsentimientoPositivo()";
     ...
     while (iteradorID_TPDU.hasNext ())
      {
       ...
       RxWindow vtaRecp = this.tablaID_SocketsEmisores.getVentanaRecepcion
           ...
           this.tablaID_SocketsEmisores.entregadoAlUsuario (id_tpdu))
         {
          ...
          try {
            ...
            */
            ...
            this.socketClusterNetImp.getCanalUnicast().send (tpduACK.construirTPDUACK(),
                                                       dirUnicastDestino);
  ...
  {
   final String mn = "DataThread.enviarTPDUMACK";
       ...
       if (this.getCGLThread().numeroVecinos()==0)
            {
             ...
             this.lTRandomMACK = (this.random.nextInt (ClusterNet.MAX_TRANDOM_MACK-ClusterNet.T_BASE)+ClusterNet.T_BASE)
                                 % ClusterNet.MAX_TRANDOM_MACK;
             this.socketClusterNetImp.getTemporizador().registrarFuncion (this.lTRandomMACK,this,
                                  ...
                                  SecuenceNumber numSecInicialRafaga)
  {
   ...
   // Formar TPDUMACK.
   try{
    TPDUMACK tpduMACK = TPDUMACK.crearTPDUMACK (this.socketClusterNetImp,
    ...
    */
    ...
    if (this.getCGLThread().numeroVecinos()>0)
      {
       this.socketClusterNetImp.getCanalMcast().send (bufAEnviar,(byte)this.getCGLThread().getTTLSocketsGL());
  ...
  {
   final String mn = "DataThread.enviarTPDUHSACK ()";
        ...
        if( treemapPadres != null)
         {
          ...
          int TTLMaxPadres = this.socketClusterNetImp.getCGLThread().
          ...
          this.socketClusterNetImp.getCanalMcast().send(bufAEnviar,(byte)TTLMaxPadres);
                                  ...
                                  ClusterGroupID idglFuente)
  {
   ...
   try{
     ...
     if( treemapPadres != null)
       {
        // Obtenemos el TTL máximo de esos padres
        int iTTLMaxPadres = this.socketClusterNetImp.getCGLThread().getTTLGLMaximo(treemapPadres);
        ...
        */
        ...
        this.socketClusterNetImp.getCanalMcast().send(bufAEnviar,(byte)iTTLMaxPadres);
  ...
  private void comprobarlistaIDs_TPDUNoRecibidoAsentimiento ()
  {
    ...
    final String mn = "DataThread.comprobarlistaIDs_TPDUNoRecibidoAsentimiento()";

    if (!((this.iMODO_FIABILIDAD==ClusterNet.MODE_RELIABLE)||(this.iMODO_FIABILIDAD==ClusterNet.MODE_DELAYED_RELIABLE)))
      return;
    ...
    while (iteradorID_TPDU.hasNext())
     {
      ...
      if (bCondicion && this.txWindow == null)
      {
        // No somos emisores.
        continue;
      }
      ...
      if (!bCondicion)
       ...
       { // No es necesario limpiar la ventana puesto que se elimina la fuente
         ...
         this.socketClusterNetImp.getCGLThread().removeID_SOCKET(id_tpdu.getID_Socket());
       }
      else
   ...
   * <b>Este método es ejecutado por el thread del {@link ClusterTimer}.</b>
  ...
  public void TimerCallback(long lEvento, Object o)
  {
   ...
   final String mn = "DataThread.TimerCallback";
  ...
  {
  final String mn = "DataThread.registrarMACK (id_tpdu,numRaf)";
   ...
   if (bEliminadoPrimerMACK)
    {
     if (this.lTRandomMACK>0)
       {
        this.lTRandomMACK = 0;
        this.socketClusterNetImp.getTemporizador().cancelarFuncion (this,this.iEVENTO_FIN_T_RANDOM_MACK);
       } // Fin de if
     if (this.vectorMACK.size()>0)
       ...
       if (this.getCGLThread().numeroVecinos()==0)
            {
             ...
             this.lTRandomMACK = (this.random.nextInt (ClusterNet.MAX_TRANDOM_MACK-ClusterNet.T_BASE)+ClusterNet.T_BASE)
                             % ClusterNet.MAX_TRANDOM_MACK;
             this.socketClusterNetImp.getTemporizador().registrarFuncion (this.lTRandomMACK,this,iEVENTO_FIN_T_RANDOM_MACK);
            }
      } // Fin del if
   } // Fin del if
  }
    ...
    for (int i=0;i<vectorNumsSecNoRecibidos.size();i++)
     {
       try{
        id_tpduAux = new ID_TPDU (id_TPDU.getID_Socket(),
                                   (SecuenceNumber)vectorNumsSecNoRecibidos.get(i));
        this.gestionAsentNeg.addID_TPDU (id_tpduAux);
       }catch (ClusterNetInvalidParameterException pie)
           {
  ...
  {
   final String mn = "DataThread.finComunicacion()";
    ...
    if (this.bORDENADO)
     {
       ...
       while (iteradorID_TPDU.hasNext())
        ...
        if ((this.txWindow==null)
             &&
            (this.socketClusterNetImp.getColaEmision().getTamaño()==0))
            return true;

        if ((this.socketClusterNetImp.getColaEmision().getTamaño()==0)
                   && this.txWindow.estaVacia())
           if ((this.iMODO_FIABILIDAD==ClusterNet.MODE_NO_RELIABLE)||(this.iMODO_FIABILIDAD==ClusterNet.MODE_NO_RELIABLE_ORDERED))
                return true;
  ...
  {
    final String mn = "DataThread.stopThread ()";
  ...
  {
   final String mn = "DataThread.actualizarListaID_TPDU2MitadRTT (id_Tpdu)";
   ...
   if ( !( (this.bFinRTT)
           ||
           (this.socketClusterNetImp.getTemporizador().getPorcentajeRTTActual () > 50) ) )
      return;
   ...
   SecuenceNumber nSecAnterior=(SecuenceNumber)this.treeMapIDs_TPDU2MitadRTT.get
  ...
  private void actualizarHSACKPendientesDeEnviar (ID_TPDU id_tpdu,int iNumeroRafaga,
                                                  boolean bPedidoRtx)
  {
   final String mn = "DataThread.actualizarHSACKPendientesDeEnviar (id_tpdu,nRaf,bPedidoRtx)";
   ...
   * indicada, según especificado en el protocolo ClusterNet.
         ...
         while (iteradorID_TPDU.hasNext())
          {
              ...
              this.socketClusterNetImp.getTemporizador().cancelarRTTID_TPDU (this,id_tpduNext);
  ...
  private boolean limpiarVentana (ID_TPDU id_tpduParam,boolean bFinOportunidades)
   {
    ...
    if (id_tpduParam.getID_Socket().equals (this.id_SocketLocal))
     ...
     {
       window = this.tablaID_SocketsEmisores.getVentanaRecepcion(id_tpduParam.getID_Socket());
   ...
   while (true)
    {
     ...
     if (!this.bABANDONAR_COMUNICACION
         ...
         bEsVtaRecepcion)
       {
        ...
        if (!window.esConsecutivo(id_tpduAnalizar.getNumeroSecuencia().tolong()))
         {
        ...
        if (!this.tablaID_SocketsEmisores.entregadoAlUsuario (id_tpduAnalizar))
        {
         if (bFinOportunidades)
           { /*
              ...
              */
              this.socketClusterNetImp.desactivarRecepcion ();

              Log.log ("SE HA CERRADO LA RECEPCION","");

              // Mandar un aviso
              this.socketClusterNetImp.sendPTMFEventError ("Se ha cerrado la recepción.");
     ...
     TPDUDatosNormal tpduDatosNormal =
          window.getTPDUDatosNormal (id_tpduAnalizar.getNumeroSecuencia().tolong());
        ...
        if (bFinOportunidades||this.tablaAsentimientos.asentido (id_tpduAnalizar))
          {
           ...
           window.removeTPDUDatosNormal (id_tpduAnalizar.getNumeroSecuencia().tolong());
           ...
           this.socketClusterNetImp.getTemporizador().cancelarRTTID_TPDUMenorIgual (this,id_tpduAnalizar);
           ...
           tpduDatosNormal = window.getPrimerTPDUConACKActivado ();
           if (tpduDatosNormal != null)
             {
 ...
 */
 private void limpiarVentana (ClusterMemberID id_socket)
 {
  ...
  TPDUDatosNormal tpduDatosNormal = window.getPrimerTPDUConACKActivado ();
 ...
 */
                                  ...
                                  Buffer bufferDatos,boolean bSetFinTransmision)
 {
  ...
  if (!getAbandonarComunicacion()) // Si se ha cerrado el socket
                                   // no entregamos más datos.
    {
     if (this.iMODO_FIABILIDAD == ClusterNet.MODE_NO_RELIABLE_ORDERED)
      {
        if (this.treeMapNSecUltimoEntregado.containsKey(id_socket))
          { // Comprobar el número de secuencia almacenado
            SecuenceNumber nSecEntregado = (SecuenceNumber)
                                this.treeMapNSecUltimoEntregado.get (id_socket);
            if (nSecEntregado.compareTo (nSec) >= 0)
              {
                // Aviso al usuario de que se ha tirado
                this.socketClusterNetImp.sendPTMFEventError ("Se han descartado datos del emisor " + id_socket + " por recibirse en desorden.");
                return; // No entregar
              }
            else // Actualizar
     ...
     this.socketClusterNetImp.getColaRecepcion().add(id_socket,bufferDatos,bSetFinTransmision);
    }
 }
  ...
  */
                                               ...
                                               boolean bRtxSoloHijosJerarquicos)
  {
   if (this.tablaAsentimientos.addID_TPDUEnEsperaAsentimiento (id_tpdu,iNumRafaga))
   {
       ...
       // oportunidades.
       this.socketClusterNetImp.getTemporizador().registrarAvisoRTT(this,this.iEVENTO_FIN_INTENTOS,id_tpdu);
    }
    else
    {
     return false;
 ...
 {
    final String mn = "DataThread.rtxTPDU";
    ...
    if ( (bRtxSiNoSincronizado || bRtxSiNoSincronizadoyEs1ªRtx)
          &&
         (this.iMODO_FIABILIDAD != ClusterNet.MODE_DELAYED_RELIABLE) )
         return false;
    ...
    if (bHayQueRtx)
     {
      ...
      try {
        Vector vectorTPDURtx =
                tpduDatosNormal.convertirAVectorTPDUDatosRtx (this.socketClusterNetImp,
                                                treeMapID_SocketsNoEnviadoACK,
                                                treeMapIDGLNoEnviadoAsentimiento);
        /* Destino:
  ...
  */
  ClusterGroupID getIDGL (ClusterMemberID id_socket)
  {
   ...
   if (id_socket.equals (this.id_SocketLocal))
        return this.socketClusterNetImp.getCGLThread().getIDGL();
  ...
  */
  ...
  {
   return this.socketClusterNetImp.getCGLThread ();
  }

} // Fin de la clase DataThread
