 ...
 * +                               |   | | | | | | | |C|T|         +<br>
 * +---------------------------------------------------------------+<br>
 * +        Tamaño Window Fuente  |     Número Ráfaga Fuente      +<br>
 * +---------------------------------------------------------------+<br>
...
public class TPDUDatosRtx extends TPDUDatos
{
   ...
   */
  private TPDUDatosRtx (SocketClusterNetImp socketClusterNetImp)
  ...
  * @exception ClusterNetInvalidParameterException si alguno de los parámetros es erróneo.
 ...
 static Vector crearVectorTPDUDatosRtx (SocketClusterNetImp socketClusterNetImp,
                                        boolean setIR,
                                        boolean setACK,
   ...
   throws ClusterNetInvalidParameterException, ClusterNetExcepcion
 {
  final String mn = "TPDUDatosRtx.crearVectorTPDUDatosRtx (socketClusterNetImp,...)";
   ...
   if (datos!=null)
     capacidad = ClusterNet.TPDU_MAX_SIZE - datos.getMaxLength() - TPDUDatosRtx.LONGHEADER;
   else capacidad = ClusterNet.TPDU_MAX_SIZE - TPDUDatosRtx.LONGHEADER;
   ...
   while (!finWhile)
   {
    ...
    if (iteradorIDGL!=null && numIDCaben > 0)
     ...
     tpduDatosRtx = new TPDUDatosRtx (socketClusterNetImp);
   ...
   */
 Buffer construirTPDUDatosRtx () throws ClusterNetExcepcion,ClusterNetInvalidParameterException
 {
  ...
  int offset = 14;
   ...
   Buffer bufferResult = this.construirCabeceraComun (ClusterNet.SUBTIPO_TPDU_DATOS_RTX,tamaño);
   ...
   throws ClusterNetExcepcion,ClusterNetInvalidParameterException
 {
  ...
  if (tpduDatosRtx.SUBTIPO != ClusterNet.SUBTIPO_TPDU_DATOS_RTX)
     throw new ClusterNetExcepcion ("Subtipo del TPDU Datos no válido");
  ...
  // 31º, 32º, 33º y 34º BYTE : Número de Secuencia Fuente
  //
  SecuenceNumber nSecFuente = new SecuenceNumber (buffer.getInt (offset));
  offset+=4;
...
//===========================================================================
 ...
 {
  return ClusterNet.TAMAÑO_VENTANA_RECEPCION;
 }
...

 {
   ...
   String result = new String (
          ...
          "\nSubtipo: " + ClusterNet.SUBTIPO_TPDU_DATOS_RTX +
