...
//------------------------------------------------------------
//
//	Fichero: RxWindow.java  1.0
//
//
//	Descripción: Clase RxWindow. Window de recepción independiente de
...
/**
 * Extiende la clase Window, redefiniendo el método de añadirTPDUDatosNormal para
 ...
 */
public class RxWindow extends Window
{
   ...
   *      métodos addNSecNoRecibido (SecuenceNumber ).
   ...
   * El vector contiene objetos SecuenceNumber.
   */
...
//  SecuenceNumber nSecEntregado ;
  ...
  */
 public RxWindow (int iTamañoMaximo,long lNSecInicial)
...
public TPDUDatosNormal addTPDUDatosNormal (long lNSec,TPDUDatosNormal pqt)
                                throws ClusterNetInvalidParameterException
{
  final String mn = "RxWindow.addTPDUDatosNormal (nSec,pqt)";
 ...
 {
   final String mn = "RxWindow.getNumsSecNoAñadidos()";
    ...
    while (lNSec<=getNumSecFinal())
     {
      if (!contieneTPDUDatosNormal(lNSec))
         vectorResult.add (new SecuenceNumber (lNSec));
  ...
  * Cada elemento del vector es un objeto SecuenceNumber.
  ...
  *
  */
 public Vector getNumsSecNoRecibidos (SecuenceNumber nSec)
 {
   final String mn = "RxWindow.getNumsSecNoRecibidos(nSec)";
   Vector vectorResult = new Vector ();
    ...
    while (lNSecCont<=lNSecMayor)
     {
      if (!contieneTPDUDatosNormal(lNSecCont))
         vectorResult.add (new SecuenceNumber (lNSecCont));
  ...
  * secuencia inicial.
  */
  public boolean recibido (SecuenceNumber nSecParam)
  {
